...
df_markets <- read_parquet("df_markets.parquet") %>%
...
server <- function(input, output, session) {
  ...
  output$inf_plot <- renderPlot({
    ...
    df_markets %>%
      ...
      summarise(
        price_change = (last(cena_na_drebno, na_rm = T) - first(cena_na_drebno, na_rm = T)) / first(cena_na_drebno, na_rm = T) * 100,
        .by = c(market, naimenovanie_na_produkta)) %>%
      filter(price_change != 0) %>%
      mutate(naimenovanie_na_produkta = fct_reorder(naimenovanie_na_produkta, price_change), col = price_change > 0,
