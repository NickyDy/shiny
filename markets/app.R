...
server <- function(input, output, session) {
  ...
  output$markets_table <- renderDT(
    ...
    df_markets %>% 
      select(date, market, kategoria, naimenovanie_na_produkta,
    ...
    df_markets %>%
      ...
      mutate(
        price_change = (last(cena_na_drebno) - first(
          cena_na_drebno)) / first(cena_na_drebno) * 100) %>%
      ungroup() %>%
      summarise(group_change = mean(price_change, na.rm = T), .by = c(market, kategoria_c)) %>%
  ...
  output$market_inf_plot <- renderPlot({
    ...
    df_market_plot() %>%
      ...
      geom_col(show.legend = F) +
      geom_text(aes(label = paste0(round(price_change, 2), "%")), hjust = -0.03) +
