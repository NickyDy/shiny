...
ppp <- read_parquet("prc_ppp_ind.parquet") %>% 
  ...
  arrange(TIME_PERIOD)
gva <- read_parquet("nama_10_a10.parquet") %>% 
  mutate(geo = fct_recode(geo, "Turkey" = "Türkiye")) %>% 
  filter(!str_detect(geo, "^Euro")) %>% 
  filter(!str_detect(geo, "^Cand")) %>% 
  arrange(TIME_PERIOD)
emp <- read_parquet("nama_10_a10_e.parquet") %>% 
  filter(!str_detect(geo, "^Euro")) %>% 
  arrange(TIME_PERIOD)
tec00009 <- read_parquet("tec00009.parquet") %>% 
  mutate(geo = fct_recode(geo, "Turkey" = "Türkiye")) %>% 
  filter(!str_detect(geo, "^Euro")) %>% 
  arrange(TIME_PERIOD)
tec00010 <- read_parquet("tec00010.parquet") %>% 
  mutate(geo = fct_recode(geo, "Turkey" = "Türkiye")) %>% 
  filter(!str_detect(geo, "^Euro")) %>% 
  arrange(TIME_PERIOD)
tec00011 <- read_parquet("tec00011.parquet") %>% 
  mutate(geo = fct_recode(geo, "Turkey" = "Türkiye")) %>% 
  filter(!str_detect(geo, "^Euro")) %>% 
  arrange(TIME_PERIOD)
#-----------------------------------------
