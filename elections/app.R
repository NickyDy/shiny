...
votes <- read_parquet("votes_new.parquet") %>%
  filter(!vote_date == "Март_2017", !party == "ИК Тодор Тодоров Батков") %>%
  mutate(party = fct_recode(party, "ИСМВ" = "ИЗПРАВИ СЕ! МУТРИ ВЪН!",
                            "ИБГНИ" = "ИЗПРАВИ СЕ БГ! НИЕ ИДВАМЕ!",
                            "ДНП" = "ДВИЖЕНИЕ НА НЕПАРТИЙНИТЕ КАНДИДАТИ")) %>% 
...
ui <- page_sidebar(
  ...
  navset_pill(
    ...
    nav_panel(title = "Карта на секцията",
              leafletOutput("map_sec", width = 1800, height = 900)),
    ...
    nav_panel(title = "Загуба/Печалба на гласове", 
              ...
              layout_columns(
              selectInput("first_date", "Последващи избори:",
                          choices = c("Април_2026",
                          "Октомври_2024",
                          "Юни_2024",
                          "Април_2023", 
                          "Октомври_2022", 
                          ...
                          "Април_2021"), 
                          selected = "Април_2026"),
              selectInput("second_date", "Предходни избори:",
                          choices = c("Април_2026", 
                                      "Октомври_2024",
                                      "Юни_2024",
                                      "Април_2023", 
...
output$map_sec <- renderLeaflet({
  sec() %>%
    left_join(., sec_coord, by = "code") %>%
    st_as_sf(coords = c("long", "lat"), crs = c(4326)) %>%
    leaflet() %>%
    addProviderTiles(providers$OpenStreetMap) %>%
    addCircles(weight = 10, color = "red") %>%
    addLabelOnlyMarkers(label =  ~ ad)
})
...
output$country <- renderPlot({
  ...
  if (input$votes_perc == "Брой гласове") {
    votes %>%
      ...
      guides(fill = guide_legend(reverse = TRUE)) +
      scale_y_discrete(labels = scales::label_wrap(50)) +
      scale_x_continuous(expand = expansion(mult = c(.05, 1.4))) +
      scale_fill_manual(values = colors) +
...
output$lost_gained_votes <- renderPlot({
  ...
  votes %>%
    mutate(party = fct_recode(party, "БСП" = "БСП-ОЛ", "ДПС" = "ДПС-НH", "ПП-ДБ" = "ДБ")) %>% 
    summarise(sum_votes = sum(votes), .by = c(vote_date, party)) %>%
...
output$text4 <- renderText({ "Панелът работи със следните филтри: Избирателен район, Община. 
  Рискът се базира на индекса за волатилност на вота (Pedersen, 1979)." })
 ...
 dt_rend() %>% 
    datatable(rownames = F,
      colnames = c("Избирателен район" = "oblast",
                   ...
                   "Секция" = "code",
                   "Риск" = "risk"), 
      options = list(dom = 'brtip', pageLength = 20)) %>% 
