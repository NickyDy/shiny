...
beef_carc_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/beef/prices?&years=2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date), 
         price = parse_number(price)) %>% 
  select(date = begin_date, state = member_state_name, category, product = product_code, price)

beef_live_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/liveAnimal/prices?years=2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = parse_number(price)) %>% 
...
piglets_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/pigmeat/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
...
pigmeat_carc_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/pigmeat/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = str_remove(price, "€"), 
         price = str_replace(price, ",", "."),
...
pigmeat_cuts_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/pigmeat/cuts/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = str_remove(price, "€"), 
         price = str_replace(price, ",", "."),
...
eggs_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/poultry/egg/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = parse_number(price)) %>% 
...
poultry_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/poultry/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = str_remove(price, "€"), 
         price = str_replace(price, ",", "."),
...
sheep_goat_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/sheepAndGoat/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date), 
         price = parse_number(price)) %>% 
  select(date = begin_date, state = member_state_name, category,
...
raw_milk_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/rawMilk/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = parse_number(price)) %>% 
...
dairy_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/dairy/prices?years=2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = parse_number(price)) %>% 
  select(date = begin_date, state = member_state_name, product,
...
oilseeds_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/oilseeds/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = parse_number(price)) %>% 
...
olive_oil_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/oliveOil/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = parse_number(price)) %>% 
...
wine_new <- fromJSON("https://www.ec.europa.eu/agrifood/api/wine/prices?beginDate=01/01/2024&endDate=31/12/2026") %>% 
  janitor::clean_names() %>% 
  mutate(begin_date = dmy(begin_date),
         price = str_remove(price, "€"), 
         price = str_replace(price, ",", "."),
